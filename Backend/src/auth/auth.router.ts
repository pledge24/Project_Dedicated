...
const loginLimiter = createRateLimiter(config.rateLimit.loginMax);
const registerLimiter = createRateLimiter(config.rateLimit.registerMax);
// 인증 필요 경로도 제한 — requireAuth가 매 호출 DB SELECT(토큰 버전 대조)를 하므로 무제한이면 증폭 벡터가 된다.
const sessionLimiter = createRateLimiter(config.rateLimit.sessionMax);
